...
!
      ...
      use icepack_parameters, only: conserv_check, restartbgc, z_tracers
      use icepack_parameters, only: skl_bgc, scale_bgc, ktherm, use_macromolecules
      use icepack_parameters, only: ratio_C2N_diatoms, ratio_C2N_sp, ratio_C2N_phaeo, &
          ratio_chl2N_diatoms, ratio_chl2N_sp, ratio_chl2N_phaeo, F_abs_chl_diatoms, &
          F_abs_chl_sp, F_abs_chl_phaeo, ratio_C2N_proteins, ratio_Si2N_diatoms, &
          ratio_Si2N_sp, ratio_Si2N_phaeo, ratio_S2N_diatoms, ratio_S2N_sp, ratio_S2N_phaeo, &
          ratio_Fe2C_diatoms, ratio_Fe2C_sp, ratio_Fe2C_phaeo, ratio_Fe2N_diatoms, &
          ratio_Fe2N_sp, ratio_Fe2N_phaeo, ratio_Fe2DON, ratio_Fe2DOC_s, ratio_Fe2DOC_l, &
          chlabs_diatoms, chlabs_sp, chlabs_phaeo, alpha2max_low_diatoms, alpha2max_low_sp, &
          alpha2max_low_phaeo, beta2max_diatoms, beta2max_sp, beta2max_phaeo, mu_max_diatoms, &
          mu_max_sp, mu_max_phaeo, grow_Tdep_diatoms, grow_Tdep_sp, grow_Tdep_phaeo, &
          fr_graze_diatoms, fr_graze_sp, fr_graze_phaeo, mort_pre_diatoms, mort_pre_sp, &
          mort_pre_phaeo, mort_Tdep_diatoms, mort_Tdep_sp, mort_Tdep_phaeo, k_exude_diatoms, &
          k_exude_sp, k_exude_phaeo, K_Nit_diatoms, K_Nit_sp, K_Nit_phaeo, K_Am_diatoms, &
          K_Am_sp, K_Am_phaeo, K_Sil_diatoms, K_Sil_sp, K_Sil_phaeo, K_Fe_diatoms, K_Fe_sp, &
          K_Fe_phaeo, f_don_protein, kn_bac_protein, f_don_Am_protein, f_doc_s, f_doc_l, &
          f_exude_s, f_exude_l, k_bac_s, k_bac_l

      use icepack_tracers, only: max_algae, max_dic, max_doc, max_don, max_fe, max_aero
      use icepack_tracers, only: ntrcr, ntrcr_o, nbtrcr
...
!
      ...
      subroutine icepack_init_zbgc(zbgc_frac_init_in, &
         zbgc_init_frac_in, tau_ret_in, tau_rel_in, bgc_tracer_type_in)

      real (kind=dbl_kind), dimension(:), optional, intent(in) :: &
         zbgc_frac_init_in(:), & ! initializes mobile fraction
         zbgc_init_frac_in(:), & ! fraction of ocean tracer  concentration in new ice
         tau_ret_in(:),        & ! retention timescale  (s), mobile to stationary phase
         tau_rel_in(:),        & ! release timescale    (s), stationary to mobile phase
         bgc_tracer_type_in(:)   ! described tracer in mobile or stationary phases
